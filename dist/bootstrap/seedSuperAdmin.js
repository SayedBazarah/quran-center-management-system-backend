...
export async function seedPermissions() {
    ...
    for (const p of PERMISSION_SEEDS) {
        const doc = await Permission.findOneAndUpdate({ code: p.code }, { $set: { name: p.name, description: p.description, isSystem: true } }, { new: true, upsert: true, setDefaultsOnInsert: true });
        codes.push(doc.code);
    }
    return codes;
}
export async function seedSuperAdminRole() {
    ...
    const role = await Role.findOneAndUpdate({ name: SUPER_ROLE_NAME }, { $set: { name: SUPER_ROLE_NAME, isSystem: true, permissions: permIds } }, { new: true, upsert: true, setDefaultsOnInsert: true });
    return { roleId: role._id };
}
export async function seedSuperAdminUser(roleId) {
    let admin = await Admin.findOne({ username: SUPER_ADMIN_USERNAME }).select("+password");
    if (!admin) {
        await Admin.create({
            id: env.admin.id,
            _id: env.admin.id,
