...
function parseListQuery(q) {
    ...
    if (q.status &&
        Object.values(StudentStatus).includes(String(q.status))) {
...
export const getStudentDetails = async (req, res, next) => {
    try {
        ...
        const student = await Student.findById(id).populate("branchId parentId adminId acceptedById firedById");
        if (!student) {
            res.status(404).json({ success: false, message: "Student not found" });
            return;
        }
...
export const listStudentsWithEnrollment = async (req, res, next) => {
    try {
        const { page, limit, skip, filter, sort } = parseListQuery(req.query);
        const latestOnly = String(req.query.latest ?? "true").toLowerCase() === "true";
...
export const updateStudentStatus = async (req, res, next) => {
    try {
        ...
        const updated = await Student.findOneAndUpdate({ _id: id, status: { $in: ["pending", status] } }, // simple transition guard
 ...
 */
export const fireStudent = async (req, res, next) => {
    try {
        const id = req.params.id;
        ...
        await student.markAsFired(firedById ? new Types.ObjectId(firedById) : new Types.ObjectId(), reason);
        res.status(200).json({
            success: true,
 ...
 */
export const reActivateFiredStudent = async (req, res, next) => {
    try {
        const id = req.params.id;
        if (!Types.ObjectId.isValid(id)) {
            res.status(400).json({ success: false, message: "Invalid student id" });
