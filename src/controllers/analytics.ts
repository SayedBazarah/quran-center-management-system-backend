...
export const globalAnalytics: RequestHandler = async (
  ...
  try {
    ...
    const [studentsByBranch, enrollmentsByBranch, enrollmentStatusCounts] =
      await Promise.all([
    ...
    const enrollmentMap = new Map(
      enrollmentsByBranch.map((e) => [String(e._id), e])
    );

    // Convert status counts array to object { active: 1, late: 2, ... }
    const statusCountMap = new Map(
      enrollmentStatusCounts.map((item) => {
        const statusObj: Record<string, number> = {};
        item.statusCounts.forEach((sc: any) => {
...
export const oldGlobalAnalytics: RequestHandler = async (
  ...
  try {
    ...
    const studentScope: Record<string, any> = { createdAt: windowCreated };
    const enrollmentScope: Record<string, any> = { createdAt: windowCreated };
    if (branchId) {
    ...
    const studentAcceptedFilter: Record<string, any> = {
      acceptedAt: { $gte: startDate, $lt: endDate },
    };
    if (branchId) studentAcceptedFilter["branchId"] = branchId;

    const enrollmentAcceptedFilter: Record<string, any> = {
      acceptedAt: { $gte: startDate, $lt: endDate },
    };
...
export const globalLogs: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
