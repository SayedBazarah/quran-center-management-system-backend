 ...
 */
StudentSchema.methods.markAsFired = async function (
  adminId: Types.ObjectId,
  reason?: string
): Promise<void> {
  this.fired = true;
  ...
  // Log the action
  await (mongoose.models.Log as mongoose.Model<ILog>).create({
    studentId: this._id,
    adminId,
 ...
 */
StudentSchema.methods.markAsGraduated = async function (): Promise<void> {
 ...
 */
StudentSchema.statics.getActiveByBranch = function (branchId: Types.ObjectId) {
  return this.find({
    branchId,
    fired: false,
 ...
 */
StudentSchema.statics.getByStatus = function (status: StudentStatus) {
